...
- (MultiLabelView *)createTableAccessoryViewOfType:(MKU_TABLEVIEW_ACCESSORY_VIEW_TYPE)type {
...
- (void)getBadge:(NSString *)badgeName {
    for (MKUBadgeItem *badge in [self updatedBadges]) {
...
- (void)updateBadge:(MKUBadgeItem *)badge {
    if ([self.badgeDelegate respondsToSelector:@selector(updateBadge:)]) {
...
- (void)setUseDarkTheme:(BOOL)useDarkTheme {
    ...
    self.sectionHeaderStyle.backgroundColor = useDarkTheme ? [UIColor blackColor] : [UIColor whiteColor];
    self.sectionHeaderStyle.textColor = useDarkTheme ? [UIColor whiteColor] : [UIColor blackColor];
}
...
//        [header.textLabel setTextColor:self.sectionHeaderTextColor];
//        [header.textLabel setFont:self.sectionHeaderFont];
//    }
//    if ([header respondsToSelector:NSSelectorFromString(@"backgroundView")]) {
//        header.backgroundView.backgroundColor = self.sectionHeaderColor;
//    }
//    if ([header respondsToSelector:NSSelectorFromString(@"contentView")]) {
...
//        footer.backgroundView.backgroundColor = [UIColor MKUMistBlue];
//    }
//}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([self isDateSection:section] && [self isEditableDateSection:section]) {
        ...
        //if (info.clearTitle && !info.showDatePicker) info.clearTitle = NO;//TODO: test use cases then delete
...
//- (void)hideAllDatePickersWithCompletion:(void (^)())completion {
//    if (!dispatch) {
//        dispatch = dispatch_queue_create("DISPATCH_QUEUE", NULL);
//    }
//
...
//    for (MKUDateViewInfoObject *info in self.dateCellInfoObjects.allValues) {
//        if (info.showDatePicker) {
//            info.showDatePicker = NO;
//            [rows addObject:[NSIndexPath indexPathForRow:1 inSection:info.indexPath.section]];
//        }
//        [sections addObject:@(info.indexPath.section)];
//    }
//            [self.tableView beginUpdates];
...
////    [self reloadSections:sections completion:completion];
//        });
//    });
//}
...
//TODO: fix this - crashes if uncommented - attempt to insert section 0 but there are only 0 sections after the update
/*
- (void)reloadDataAnimated:(BOOL)animated {
    //dispatch_async(dispatch, ^{
    dispatch_async(dispatch_get_main_queue(), ^{
        if (animated) {
            ...
            //}
            //
            //NSIndexSet *sections = [NSIndexSet indexSetWithIndexesInRange:NSMakeRange(paths.firstObject.section, paths.count - 1)];
            //if (sections.count <= [self.tableView numberOfSections]) {
            //    [self.tableView beginUpdates];
            //    [self.tableView reloadSections:sections withRowAnimation:UITableViewRowAnimationAutomatic];
            //    [self.tableView endUpdates];
            //}
...
//TODO: This doesn't work properly, causes UI glitch in other sections
...
- (void)createTableFooterWithTitle:(NSString *)title {
    ...
    if (title.length > 0) {
        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake([Constants TableCellContentHorizontalMargin], [Constants TableCellContentVerticalMargin], self.view.frame.size.width-2*[Constants TableCellContentVerticalMargin], [Constants TableFooterHeight])];
        ...
        label.textColor = [AppTheme sectionFooterTextColor];
    ...
    if (info.showDatePicker) {
        switch (info.indexPath.row) {
            case MKU_DATE_PICKER_ROW_LABEL: {
                ...
                if (!cell) {
                    cell = [[MKUDateTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:[MKUDateTableViewCell identifier]];
                }
            ...
            case MKU_DATE_PICKER_ROW_PICKER: {
                MKUSingleViewTableViewCell *cell = [[MKUSingleViewTableViewCell alloc] initWithInsets:UIEdgeInsetsMake([Constants TableCellContentVerticalMargin], [Constants TableCellContentHorizontalMargin], [Constants TableCellContentVerticalMargin], [Constants TableCellContentHorizontalMargin]) viewCreationHandler:^UIView *{
...
- (CGFloat)heightForTextFieldCellAtIndexPath:(NSIndexPath *)indexPath {
    return [Constants TextFieldHeight] + 2*[Constants TableCellContentVerticalMargin];
}
