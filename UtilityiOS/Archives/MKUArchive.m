...
@interface MKUArchive ()

@property (nonatomic, strong, readwrite) NSString *path;

@end

@implementation MKUArchive
...
- (NSString *)nextDataPath {
    return [MKUArchiveDataBase nextDataPathOfClass:[self class] prefix:self.directoryPrefix];
}
...
@implementation MKUArchiveDataBase
...
+ (NSMutableArray<MKUArchive *> *)loadDataOfClass:(Class)docClass {
    NSString *documentsDirectory = [MKUArchiveDataBase privateDocsDir];
...
+ (void)deleteAllDocs {
    NSError *error;
    BOOL success = [[NSFileManager defaultManager] removeItemAtPath:[MKUArchiveDataBase privateDocsDir] error:&error];
    if (!success) {
        DEBUGLOG(@"Error removing document path: %@", error.localizedDescription);
    }
}
